...
const getIsPremium = async ({ userId }) => {
  const result = await db.query('SELECT is_premium FROM users WHERE id = $1', [userId]);
...
exports.buscarRetrospectiva = async ({ userId, period = 'yearly', lang = 'pt' }) => {
  const safePeriod = ['weekly', 'monthly', 'yearly'].includes(period) ? period : 'yearly';
  const safeLang = lang === 'en' ? 'en' : 'pt';
  const { startDate, endDate } = resolvePeriodRange(safePeriod);
  ...
  const treinosResult = await db.query(treinosQuery, [userId, startDate, endDate]);
  const treinos = treinosResult.rows;
  ...
  const detalhesQuery = `
    ...
    WHERE et.treino_id = ANY($2::int[])
      AND t.user_id = $1
    ORDER BY t.data DESC, et.ordem ASC, s.numero ASC
  `;
  ...
  const treinosList = Array.from(treinosMap.values());
  const summary = treinosList.reduce(
    (acc, treino) => {
      acc.total_treinos += 1;
      acc.total_series += Number(treino.total_series || 0);
      acc.duracao_total += Number(treino.duracao || 0);
      acc.peso_total += Number(treino.peso_total || 0);
      acc.total_exercicios += Array.isArray(treino.exercicios) ? treino.exercicios.length : 0;
      return acc;
    },
    {
      total_treinos: 0,
